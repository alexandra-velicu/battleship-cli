...
describe('Player.configureShip', () => {
    ...
    it('should return error message for invalid ship', () => {
        const result = player.configureShip('invalidShip', 'A1', 'right', false);
        expect(result).toBe(INVALID.SHIP);
    });

    it('should return error message for invalid direction', () => {
        const result = player.configureShip('cruiser', 'A1', 'diagonal', false);
